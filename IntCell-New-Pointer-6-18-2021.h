class IntCell
{
    public:
        ...
        /*IntCell Read method used for returning all non zero values stored 
        in public or private variables*/
        int Read() const
        {
            if(storeValue == 0) { return temp; } else { return storeValue; }
        }//end of Read
        
        /*IntCell Write method used for chancing either the temp variable 
        if the value is not equal to zero or the currently stored value*/
        void Write( int initialValue )
        {
            if(storeValue == 0 && initialValue != 0) { temp = initialValue; } else { storeValue = initialValue; }
        }//end of Write
...
{
   /*declaring pointers*/
   IntCell *objTest1 = new IntCell(5);
   IntCell *objTest2 = new IntCell;
